...
model.load_weights('./retina_Unet_150epochs.hdf5') #loading weights


path1 = './xxx/test_img'    #test dataset images directory path

path2 = './xxx/test_gt'     #test dataset mask directory path
...
for idx, image_name in enumerate(testimages):  
   if image_name.endswith(".png"): 
      ...
      groundtruth=[]
      groundtruth = skimage.io.imread(path2+'/'+testmasks[idx],as_gray=True) #reading mask of the test img      
      ...
      IoU = []
      for label in labels:
          jaccard = jaccard_score((y_pred.flatten()).astype(int),(y_true.flatten()).astype(int), pos_label=label, average='weighted')
          IoU.append(jaccard)
      ...
      cm=[]
      accuracy = []
      cm = confusion_matrix((y_true.flatten()).astype(int),(y_pred.flatten()).astype(int), labels=[0, 1])
      accuracy = (cm[0,0]+cm[1,1])/(cm[0,0]+cm[0,1]+cm[1,0]+cm[1,1]) #accuracy of single image
      print(prediction[c].shape)
      img = cv2.convertScaleAbs(prediction[c], alpha=(255.0)) ##### 這邊回到 0-255
      cv2.imwrite('./pre_img/pre_%s.png'%(str(c)),img)
      c+=1
...
reconstructed_image = []
test_img = skimage.io.imread('F:\\nuck\\code\\xxx\\test_img\\05.png') #test image
